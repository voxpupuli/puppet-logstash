...
#  http://logstash.net/docs/1.2.2/filters/ruby
#
...
define logstash::filter::ruby (
  ...
  if ($code != '') {
    ...
    $opt_code = "${opt_indent}code => \"${code}\"\n"
  }

  if ($init != '') {
    validate_string($init)
    $opt_init = "${opt_indent}init => \"${init}\"\n"
  }
  ...
  file { $conffiles:
    ...
    content => "filter {\n${opt_cond_start} ruby {\n${opt_add_field}${opt_add_tag}${opt_code}${opt_exclude_tags}${opt_init}${opt_remove_tag}${opt_tags}${opt_type}${opt_cond_end}}\n}\n",
    mode    => '0440',
