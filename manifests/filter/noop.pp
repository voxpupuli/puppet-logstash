...
#   Example:  filter {   noop {     add_field =&gt; [ "sample", "Hello
#   world, from %{@source}" ]   } }    On success, the noop plugin
...
#   syntax. Example:  filter {   noop {     add_tag =&gt; [
...
#   syntax. Example:  filter {   noop {     remove_tag =&gt; [
#   "foo_%{somefield}" ]   } }   If the event has field "somefield" ==
...
#  http://logstash.net/docs/1.2.2/filters/noop
#
...
define logstash::filter::noop (
  ...
  if $logstash::multi_instance == true {
    ...
    $conffiles    = suffix($confdirstart, "/config/filter_${order}_noop_${name}")
  ...
  file { $conffiles:
    ...
    content => "filter {\n${opt_cond_start} noop {\n${opt_add_field}${opt_add_tag}${opt_exclude_tags}${opt_remove_tag}${opt_tags}${opt_type}${opt_cond_end}}\n}\n",
    mode    => '0440',
