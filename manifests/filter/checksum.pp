...
#  http://logstash.net/docs/1.2.2/filters/checksum
#
...
define logstash::filter::checksum (
  ...
  if ($keys != '') {
    validate_array($keys)
    $arr_keys = join($keys, '\', \'')
    $opt_keys = "${opt_indent}keys => ['${arr_keys}']\n"
  }
  ...
  file { $conffiles:
    ...
    content => "filter {\n${opt_cond_start} checksum {\n${opt_add_field}${opt_add_tag}${opt_algorithm}${opt_exclude_tags}${opt_keys}${opt_remove_tag}${opt_tags}${opt_type}${opt_cond_end}}\n}\n",
    mode    => '0440',
