...
#  http://logstash.net/docs/1.2.2/filters/translate
#
...
define logstash::filter::translate (
  ...
  if ($exact != '') {
    validate_bool($exact)
    $opt_exact = "${opt_indent}exact => ${exact}\n"
  }

  if ($override != '') {
    validate_bool($override)
    $opt_override = "${opt_indent}override => ${override}\n"
  }

  if ($regex != '') {
    validate_bool($regex)
    $opt_regex = "${opt_indent}regex => ${regex}\n"
  }
  ...
  if ($dictionary != '') {
    ...
    $opt_dictionary = "${opt_indent}dictionary => ${arr_dictionary}\n"
  }
  ...
  if ($dictionary_path != '') {
    if $dictionary_path =~ /^puppet\:\/\// {
      ...
      $opt_dictionary_path = "${opt_indent}dictionary_path => \"${filesdir}/${basefilename_dictionary_path}\"\n"
      ...
      $opt_dictionary_path = "${opt_indent}dictionary_path => \"${dictionary_path}\"\n"
    }
  }
  ...
  if ($fallback != '') {
    ...
    $opt_fallback = "${opt_indent}fallback => \"${fallback}\"\n"
  }
  ...
  file { $conffiles:
    ...
    content => "filter {\n${opt_cond_start} translate {\n${opt_add_field}${opt_add_tag}${opt_destination}${opt_dictionary}${opt_dictionary_path}${opt_exact}${opt_exclude_tags}${opt_fallback}${opt_field}${opt_override}${opt_regex}${opt_remove_tag}${opt_tags}${opt_type}${opt_cond_end}}\n}\n",
    mode    => '0440',
    notify  => Service[$services],
