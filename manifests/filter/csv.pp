...
#  http://logstash.net/docs/1.2.2/filters/csv
#
...
define logstash::filter::csv (
  ...
  if ($columns != '') {
    ...
    $opt_columns = "${opt_indent}columns => ['${arr_columns}']\n"
  }
  ...
  if ($separator != '') {
    ...
    $opt_separator = "${opt_indent}separator => \"${separator}\"\n"
  }
  ...
  file { $conffiles:
    ...
    content => "filter {\n${opt_cond_start} csv {\n${opt_add_field}${opt_add_tag}${opt_columns}${opt_exclude_tags}${opt_remove_tag}${opt_separator}${opt_source}${opt_tags}${opt_target}${opt_type}${opt_cond_end}}\n}\n",
    mode    => '0440',
