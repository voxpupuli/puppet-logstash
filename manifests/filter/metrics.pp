...
#  http://logstash.net/docs/1.2.2/filters/metrics
#
...
define logstash::filter::metrics (
  ...
  if ($ignore_older_than != '') {
    if ! is_numeric($ignore_older_than) {
      ...
      $opt_ignore_older_than = "${opt_indent}ignore_older_than => ${ignore_older_than}\n"
    }
  }
  ...
  file { $conffiles:
    ...
    content => "filter {\n${opt_cond_start} metrics {\n${opt_add_field}${opt_add_tag}${opt_exclude_tags}${opt_ignore_older_than}${opt_meter}${opt_remove_tag}${opt_tags}${opt_timer}${opt_type}${opt_cond_end}}\n}\n",
    mode    => '0440',
