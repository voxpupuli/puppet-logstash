...
#  http://logstash.net/docs/1.2.2/filters/zeromq
#
...
define logstash::filter::zeromq (
  ...
  if ($address != '') {
    ...
    $opt_address = "${opt_indent}address => \"${address}\"\n"
  }
  ...
  file { $conffiles:
    ...
    content => "filter {\n${opt_cond_start} zeromq {\n${opt_add_field}${opt_add_tag}${opt_address}${opt_exclude_tags}${opt_field}${opt_mode}${opt_remove_tag}${opt_sockopt}${opt_tags}${opt_type}${opt_cond_end}}\n}\n",
    mode    => '0440',
