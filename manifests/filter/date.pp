...
#  http://logstash.net/docs/1.2.2/filters/date
#
...
define logstash::filter::date (
  ...
  if ($locale != '') {
    ...
    $opt_locale = "${opt_indent}locale => \"${locale}\"\n"
  }
  ...
  file { $conffiles:
    ...
    content => "filter {\n${opt_cond_start} date {\n${opt_add_field}${opt_add_tag}${opt_exclude_tags}${opt_locale}${opt_match}${opt_remove_tag}${opt_tags}${opt_type}${opt_cond_end}}\n}\n",
    mode    => '0440',
