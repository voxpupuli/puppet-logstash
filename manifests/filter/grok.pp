...
define logstash::filter::grok (
  ...
  if ($tag_on_failure != '') {
    ...
    $opt_tag_on_failure = "${opt_indent}tag_on_failure => ['${arr_tag_on_failure}']\n"
  }
  ...
  if ($pattern != '') {
    ...
    $opt_pattern = "${opt_indent}pattern => ['${arr_pattern}']\n"
  }

  if ($named_captures_only != '') {
    validate_bool($named_captures_only)
    $opt_named_captures_only = "${opt_indent}named_captures_only => ${named_captures_only}\n"
  }

  if ($singles != '') {
    validate_bool($singles)
    $opt_singles = "${opt_indent}singles => ${singles}\n"
  }

  if ($keep_empty_captures != '') {
    validate_bool($keep_empty_captures)
    $opt_keep_empty_captures = "${opt_indent}keep_empty_captures => ${keep_empty_captures}\n"
  }

  if ($drop_if_match != '') {
    validate_bool($drop_if_match)
    $opt_drop_if_match = "${opt_indent}drop_if_match => ${drop_if_match}\n"
  }

  if ($break_on_match != '') {
    validate_bool($break_on_match)
    $opt_break_on_match = "${opt_indent}break_on_match => ${break_on_match}\n"
  }
  ...
  file { $conffiles:
    ...
    content => "filter {\n${opt_cond_start} grok {\n${opt_add_field}${opt_add_tag}${opt_break_on_match}${opt_drop_if_match}${opt_exclude_tags}${opt_keep_empty_captures}${opt_match}${opt_named_captures_only}${opt_pattern}${opt_patterns_dir}${opt_remove_tag}${opt_singles}${opt_tag_on_failure}${opt_tags}${opt_type}${opt_cond_end}}\n}\n",
    mode    => '0440',
