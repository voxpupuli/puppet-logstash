...
#   interface, you'll need to use this output.    NOTE: The elasticsearch
#   client is version 0.19.8. Your elasticsearch   cluster must be running
#   0.19.x for API compatibility.  If you want to set other elasticsearch
#   options that are not exposed directly as config options, there are two
#   options: * create an elasticsearch.yml file in the $PWD of the
#   logstash process * pass in es.* java properties (java -Des.node.foo=
#   or ruby -J-Des.node.foo=)  You can learn more about elasticsearch at
