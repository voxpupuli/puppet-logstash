# == Define: logstash::output::null
#
#   A null output. This is useful for testing logstash inputs and filters
#   for performance.
#
#
# === Parameters
...
#  http://logstash.net/docs/1.2.2/outputs/null
#
...
define logstash::output::null (
  ...
  if $logstash::multi_instance == true {
    ...
    $conffiles    = suffix($confdirstart, "/config/output_null_${name}")
    $services     = prefix($instances, 'logstash-')
    $filesdir     = "${logstash::configdir}/files/output/null/${name}"

  } else {

    $conffiles = "${logstash::configdir}/conf.d/output_null_${name}"
    $services  = 'logstash'
    $filesdir  = "${logstash::configdir}/files/output/null/${name}"
  ...
  file { $conffiles:
    ...
    content => "output {\n${opt_cond_start} null {\n${opt_exclude_tags}${opt_fields}${opt_codec}${opt_tags}${opt_type}${opt_cond_end}}\n}\n",
    mode    => '0440',
