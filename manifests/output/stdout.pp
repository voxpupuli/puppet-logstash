...
#  http://logstash.net/docs/1.2.2/outputs/stdout
#
...
define logstash::output::stdout (
  ...
  if ($debug_format != '') {
    if ! ($debug_format in ['ruby', 'json', 'dots']) {
      ...
      $opt_debug_format = "${opt_indent}debug_format => \"${debug_format}\"\n"
    }
  }
  ...
  file { $conffiles:
    ...
    content => "output {\n${opt_cond_start} stdout {\n${opt_debug}${opt_debug_format}${opt_exclude_tags}${opt_fields}${opt_codec}${opt_message}${opt_tags}${opt_type}${opt_cond_end}}\n}\n",
    mode    => '0440',
