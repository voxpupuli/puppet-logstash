...
#
#   Run command line tools and cature output as an event.  Notes:  The
#   '@source' of this event will be the command run. The '@message' of
#   this event will be the entire stdout of the command as one event.
#   TODO(sissel): Implement a 'split' filter so you can split output of
#   this and other messages by newline, etc.
#
#
# === Parameters
#
...
#  http://logstash.net/docs/1.1.5/inputs/exec
#
...
define logstash::input::exec(
  ...
  file { "${logstash::params::configdir}/input_exec_${name}":
    ...
    content => "input {\n exec {\n${opt_add_field}${opt_command}${opt_debug}${opt_format}${opt_interval}${opt_message_format}${opt_tags}${opt_type} }\n}\n",
    owner   => 'root',
    group   => 'root',
    mode    => '0644',
