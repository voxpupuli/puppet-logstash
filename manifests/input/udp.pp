...
#  http://logstash.net/docs/1.2.2/inputs/udp
#
...
define logstash::input::udp (
  ...
  if ($buffer_size != '') {
    if ! is_numeric($buffer_size) {
      ...
      $opt_buffer_size = "${opt_indent}buffer_size => ${buffer_size}\n"
    }
  }
  ...
  file { $conffiles:
    ...
    content => "input {\n${opt_cond_start} udp {\n${opt_add_field}${opt_buffer_size}${opt_debug}${opt_host}${opt_codec}${opt_port}${opt_tags}${opt_type}${opt_cond_end}}\n}\n",
    mode    => '0440',
