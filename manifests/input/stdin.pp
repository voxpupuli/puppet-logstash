...
#  http://logstash.net/docs/1.1.5/inputs/stdin
#
...
define logstash::input::stdin(
  ...
  file { "${logstash::params::configdir}/input_stdin_${name}":
    ensure  => present,
    content => "input {\n stdin {\n${opt_add_field}${opt_debug}${opt_format}${opt_message_format}${opt_tags}${opt_type} }\n}\n",
    owner   => 'root',
    group   => 'root',
    mode    => '0644',
