...
#  http://logstash.net/docs/1.1.5/inputs/ganglia
#
...
define logstash::input::ganglia(
  ...
  file { "${logstash::params::configdir}/input_ganglia_${name}":
    ensure  => present,
    content => "input {\n ganglia {\n${opt_add_field}${opt_debug}${opt_format}${opt_host}${opt_message_format}${opt_port}${opt_tags}${opt_type} }\n}\n",
    owner   => 'root',
    group   => 'root',
    mode    => '0644',
