...
#  http://logstash.net/docs/1.1.5/inputs/twitter
#
...
define logstash::input::twitter(
  ...
  file { "${logstash::params::configdir}/input_twitter_${name}":
    ensure  => present,
    content => "input {\n twitter {\n${opt_add_field}${opt_debug}${opt_format}${opt_keywords}${opt_message_format}${opt_password}${opt_tags}${opt_type}${opt_user} }\n}\n",
    owner   => 'root',
    group   => 'root',
    mode    => '0644',
