...
#  http://logstash.net/docs/1.1.5/inputs/pipe
#
...
define logstash::input::pipe(
  ...
  file { "${logstash::params::configdir}/input_pipe_${name}":
    ensure  => present,
    content => "input {\n pipe {\n${opt_add_field}${opt_command}${opt_debug}${opt_format}${opt_message_format}${opt_tags}${opt_type} }\n}\n",
    owner   => 'root',
    group   => 'root',
    mode    => '0644',
