...
#  http://logstash.net/docs/1.1.10/inputs/lumberjack2
#
...
define logstash::input::lumberjack2 (
  ...
  if $their_public_key {
    if $their_public_key =~ /^puppet\:\/\// {
      ...
      $filenameArray = split($their_public_key, '/')
      $basefilename = $filenameArray[-1]

      $opt_their_public_key = "  their_public_key => \"${filesdir}/${basefilename}\"\n"
      ...
      $opt_their_public_key = "  their_public_key => \"${their_public_key}\"\n"
    }
  }
  ...
  if $my_secret_key {
    if $my_secret_key =~ /^puppet\:\/\// {
      ...
      $filenameArray = split($my_secret_key, '/')
      $basefilename = $filenameArray[-1]

      $opt_my_secret_key = "  my_secret_key => \"${filesdir}/${basefilename}\"\n"
      ...
      $opt_my_secret_key = "  my_secret_key => \"${my_secret_key}\"\n"
    }
  }
